...
def validator(
    db: Session, og_url: schemas.UrlCreate, slot: str, c_slot=compact_slot
) -> bool:
    # return True if slot is empty
    # and item if slot doesn't equal the key
    if get_url_byshorten(db, c_slot(slot)):  # check if slot is occupied
        return False
    if url := get_url_byoriginal(
        db, og_url
    ):  # always false if used only inside create_shorten_link
...
def generate_short_link(
    db: Session, og_url: schemas.UrlCreate, val=validator, c_slot=compact_slot
) -> str:
...
def inc_number_of_cliks(db: Session, shorten_url: str):
    db.query(models.Url).filter(models.Url.shorten_url == shorten_url).update(
        {"clicks": models.Url.clicks + 1}
    )  # update number of cliks
    db.commit()
...
def get_url_byoriginal(db: Session, original_url: schemas.UrlCreate):
    return (
        db.query(models.Url)
        .filter(models.Url.original_url == str(original_url.original_url))
        .first()
    )
