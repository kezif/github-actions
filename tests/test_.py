...
'''
@pytest.mark.parametrize('long_url', ['https://en.wikipedia.org/wiki/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connectori/Atlantic_City%E2%80%93Brigantine_Connector'])
...
@pytest.mark.parametrize('link', ['https://github.com/', 'https://github.com/kezif/fabric', 'https://www.linkedin.com/'])
class TestShortening():
    ...
    def test_shorten_to_original_mapping(self, link):
        ...
        res = requests.get(SITE_URL + f'/{shorten_url_memory}', verify=False, headers=headers)
        assert link == res.json()['original_url'], f'shorten url doesnt correlate with original url {res.json()}  og:{link}'
        

    def test_short_info(self, link):
        shorten_url_memory = self.mapping[link]
        res = requests.get(SITE_URL + f'/info/{shorten_url_memory}', verify=False, headers=headers)
        jres = res.json()
        assert jres["original_url"] is not None, f"Original URL not found in the response: {res.json()}"
        assert jres["shorten_url"] is not None, f"Shorten URL not found in the response: {res.json()}"
        assert jres["clicks"] is not None, f"Cliks URL not found in the response: {res.json()}"
        assert jres["created_at"] is not None, f"Date of creation URL not found in the response: {res.json()}"
